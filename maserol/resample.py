...
from maserol.core import reshape_params, optimize_loss, DEFAULT_AB_TYPES
...
def resample(cube: xr.DataArray, replace=True):
    ...
    sample_idx = np.random.choice(
        cube.sizes["Sample"], replace=replace, size=cube.sizes["Sample"]
    )
...
def bootstrap(
    cube: xr.DataArray,
    post_opt_factor=False,
    numResample=10,
    norm_cols=True,
    norm: Optional[Literal["l2", "max"]] = None,
    **opt_kwargs
):
    """
    ...
    for _ in range(numResample):
        data = resample(cube)
        x, _ = optimize_loss(data, **opt_kwargs)
        x = reshape_params(
            x,
            data,
            opt_kwargs["lrank"],
            opt_kwargs["fitKa"],
            ab_types=opt_kwargs.get("ab_types", DEFAULT_AB_TYPES),
            as_xarray=post_opt_factor,
        )
    ...
    return mean_std(samp_list), mean_std(ag_list) if opt_kwargs[
        "lrank"
    ] or post_opt_factor else mean_std(abundance_list)
