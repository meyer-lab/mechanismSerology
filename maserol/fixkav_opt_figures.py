...
def configure_scatterplot(data : xr.DataArray, lbound, loc=None, antigens=False, log=False): 
    """
    ...
    antigen_labels = antigen_labels if antigens else None

    lbound_flat = lbound.flatten()[non_nan]
    cube_flat = cube_flat[non_nan]
    print(np.unique(np.isnan(cube_flat)))
    print(np.unique(np.isnan(lbound_flat)))
    receptor_labels = receptor_labels[non_nan]
    if antigens:
        antigen_labels = antigen_labels[non_nan]

    # plot
    f = sns.scatterplot(x=np.log10(cube_flat), y=np.log10(lbound_flat), hue=receptor_labels, style=antigen_labels, ax=loc, s=70, alpha=0.5)
    ...
    f.set_xlabel("log10 (Actual)", fontsize=12)
    f.set_ylabel("log10 (Predictions)", fontsize=12)
    return f

def make_initial_final_lbound_correlation_plot(cube, initial_lbound, final_lbound, per_receptor, antigens=False):
    """
    ...
    initial_f = configure_scatterplot(cube, initial_lbound, axs[0], antigens)
    initial_f.set_title("Initial", fontsize=13)
    final_f = configure_scatterplot(cube, final_lbound, axs[1], antigens)
    final_f.set_title("After Abundance Fit", fontsize=13)
    add_r_text(cube, initial_lbound, final_lbound, per_receptor, f, antigens)
    return f

def add_r_text(cube, initial_lbound, final_lbound, per_receptor, f, antigens=False):
    """
    Prints r value of each receptor and the average r value on the plot f.
    """
    cube_flat = (prepare_data(cube)).values.flatten()
    nonzero = ~np.isnan(cube_flat)
