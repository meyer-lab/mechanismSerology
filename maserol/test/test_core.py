...
def test_initialize(n_ab):
    ...
    ab_types = HIgGs[:n_ab]
    ps = initialize_params(cube, ab_types=ab_types)  # should return abund, Ka
    assert len(ps) == 2
...
def test_inferLbound_matches_valentbind():
    ...
    Rtot = np.random.rand(n_subj, n_ab, n_ag) * np.power(
        10, np.random.randint(1, 5, size=(n_subj, n_ab, n_ag))
    )
    Ka = np.random.rand(n_rcp, n_ab) * np.power(
        10, np.random.randint(5, 8, size=(n_rcp, n_ab))
    )
...
def test_inferLbound_monotonicity():
    ...
    Rtot = xr.DataArray(
        Rtot_np,
        [np.arange(Rtot_np.shape[0]), list(ab_types), np.arange(Rtot_np.shape[2])],
        ["Sample", "Antibody", "Antigen"],
    )
    rcp = np.array(["IgG1", "FcgRI"])
    cube = xr.DataArray(
        np.zeros((Rtot.shape[0], rcp.size, Rtot.shape[2])),
        (Rtot.Sample.values, rcp, Rtot.Antigen.values),
    ...
    assert is_monotonically_increasing(
        np.log10(Rtot_np.flatten()), np.log10(Lbound[:, 0, :])
    )
    assert is_monotonically_increasing(
        np.log10(Rtot_np.flatten()), np.log10(Lbound[:, 1, :])
    )


@pytest.mark.parametrize(
    "data", [atyeo(), SpaceX4D().stack(Sample=("Subject", "Time"))]
)
def test_reshape_params(data):
    ab_types = list(HIgGs)
    cube = prepare_data(data)
    abundance, Ka = initialize_params(cube, ab_types=ab_types)
    x = flatten_params(abundance, Ka)
    abundance, Ka = reshape_params(
        x, cube, fitKa=True, as_xarray=True, ab_types=ab_types
    )
...
def test_forward_backward_simple(n_samp, L0, rcp_high):
    ...
    cube = xr.DataArray(
        np.zeros((Rtot.shape[0], len(rcp), Rtot.shape[2])),
        (Rtot.Sample.values, rcp, Rtot.Antigen.values),
        ("Sample", "Receptor", "Antigen"),
    )
    Ka = assemble_Ka(cube, ab_types)
...
def test_fit_Ka():
    ...
    x_opt, _ = optimize_loss(cube, L0, KxStar, f, ab_types, fitKa=fitKa)
    Rtot, Ka_fitted = reshape_params(x_opt, cube, fitKa=fitKa, ab_types=ab_types)
    Ka_after = Ka.copy()
