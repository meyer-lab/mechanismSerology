...
def plot_deviation_heatmap(mean_matrix, std_matrix, absf, ylabels):
    ...
    """
    lower_map = np.asarray(
        [mean_matrix[i] - std_matrix[i] for i in range(len(mean_matrix))]
    )
    upper_map = np.asarray(
        [mean_matrix[i] + std_matrix[i] for i in range(len(mean_matrix))]
    )
    M = len(absf)
    N = lower_map.shape[0]
...
def removed_weights_subj_ag(cube, subjects, antigens, absf):
    ...
    fig, axes = plt.subplots(
        nrows=2, ncols=2, gridspec_kw={"height_ratios": [1, 5]}, figsize=(10, 12)
    )
    axes[0, 1].axis("off")
    cbar_ax = fig.add_axes([1, 0.047, 0.03, 0.72])
    ...
    sb = sns.barplot(
        ...
        x=list(absf),
        ax=axes[0, 0],
        palette=np.asarray(spal)[::-1][srank],
    )
    sb.set_xticklabels([])
    sb.set_yticklabels([])
