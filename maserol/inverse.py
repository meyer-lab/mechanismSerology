...
from .core import infer_Lbound, assemble_Ka, optimize_loss
...
def genForwardSim(noise=0.5, n_sample=1000, n_antigen=5):
    ...
    cube = xr.DataArray(
        np.zeros((Rtot.shape[0], len(ab_types + fcr_types), Rtot.shape[2])),
...
def plotInverse():
    ...
    x, _ = optimize_loss(
        cube, fitKa=False, ab_types=tuple(ab_types), L0=L0, KxStar=KxStar
    )
    ...
    sns.scatterplot(
        ax=ax[0],
        x=np.log10(Rtot.values.flatten()),
        y=np.log10(np.exp(x)),
        hue=Rtot_df["Antibody"],
        style=Rtot_df["Antigen"],
    )
