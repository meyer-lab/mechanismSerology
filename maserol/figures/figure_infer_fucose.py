...
def makeFigure():
    ...
    abundance_list = plot_ab_aggs(
        cube_alter.sel(Antigen=gp120_ag, Sample=gp120["subject"].values),
        abs,
        3,
        ax=axes[0],
    )
    ...
    get_aggs = lambda abs: np.sum(
        np.sum(
            np.mean(
                np.array([abund.sel(Antibody=abs) for abund in abundance_list]), axis=0
            ),
            axis=2,
        ),
        axis=1,
