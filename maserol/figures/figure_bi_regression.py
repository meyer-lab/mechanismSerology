...
def makeFigure():
    ...
    ab_types = HIgGFs
    x_opt_lrank, _ = optimize_loss(cube, lrank=True, ab_types=ab_types)
    sample, ag = reshape_params(x_opt_lrank, cube, lrank=True, ab_types=ab_types)

    ab_types = HIgGs
    x_opt_lrank_no_fucose, _ = optimize_loss(cube, lrank=True, ab_types=ab_types)
    sample_no_fucose, _ = reshape_params(
        x_opt_lrank_no_fucose, cube, lrank=True, ab_types=ab_types
    )

    labels, label_encoder = get_labels_zohar(multiclass=False)
    y_pred, model, x, y = regression(sample, labels, scale_x=0, l1_ratio=0)
    y_pred_no_fucose, model_no_fucose, x_no_fucose, y = regression(
        sample_no_fucose, labels, scale_x=0, l1_ratio=0
    )
